...
if ( is_singular( get_post_type() ) ) {
	...
	if ( ! empty( $post->post_parent ) ) {
		...
		if ( ! empty( $prev_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Previous:', 'qualityroof-theme' ) . '</span></div>';
		...
		if ( ! empty( $next_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Next:', 'qualityroof-theme' ) . '</span></div>';
			$attachment_navigation .= $next_link;
			$attachment_navigation .= '</div>';
		}

		if ( ! empty( $attachment_navigation ) ) {
			echo _navigation_markup( $attachment_navigation, $class = 'post-navigation', __( 'Post navigation', 'qualityroof-theme' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
